...
function isAllowedLine(relPath, line) {
  if (relPath !== "src/helpers/logout.ts") return false
  return /^(CSRF|AUTH_LOGOUT):\s*\"\/v2\//.test(line.trim())
}
...
if (findings.length > 0) {
  console.error("Found disallowed secure endpoint literals in SDK React source:")
  for (const f of findings) console.error(`- ${f}`)
  process.exit(1)
}

console.log("Secure route guard passed for SDK React.")
